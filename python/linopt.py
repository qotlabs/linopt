...
if sys.argv[1] == 'BFGS':
	opt_val = optimize.minimize(linoptlib.cost_function, 6*np.random.rand(num_modes**2) - 3, method='BFGS', jac=False, tol=1e-3, callback=None, options=None)
	...
	opt_val = optimize.basinhopping(linoptlib.cost_function, 6*np.random.rand(num_modes**2) - 3, niter=1000, T=0.02, stepsize=0.5, minimizer_kwargs={"method":"BFGS"}, take_step=None, accept_test=None, callback=print_fun, interval=50, disp=False, niter_success=None)
elif sys.argv[1] == 'SDA':
    bounds = np.tile(np.array([0,1]),(num_modes**2,1))
    opt_val = sda(linoptlib.cost_function, np.random.rand(num_modes**2), bounds, maxiter=1000, minimizer_kwargs={"method":"BFGS"}, initial_temp=10, visit=2.62, accept=-10.0, maxfun=1e7, seed=None, pure_sa=False)
else:
