...
linopt = ctypes.CDLL('../wrappers/pylinopt.so')

# low-level log_functor constructor
linopt.log_functor_constructor.argtypes = (ctypes.c_int, ctypes.c_int, # Number of full system and ancilla modes
...
linopt.log_functor_constructor.restype = ctypes.c_void_p

# log_functor constructor with hard-coded initialization parameters
# Mf = 8, Ma = 4, full number of photons - 4, target states - 6 dual-rail Bell states
sf_constructor = linopt.log_functor_simple_constructor
sf_constructor.restype = ctypes.c_void_p

# log_functor destructor
sf_destructor = linopt.log_functor_destructor
sf_destructor.argtype = ctypes.c_void_p

# cost function implementation based on the method described in log et al. Generating entanglement with linear optics, PRA 96, 043861 (2017)
...
costfunc = linopt.log_functor_apply
